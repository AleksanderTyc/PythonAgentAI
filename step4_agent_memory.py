...
system_prompt = ("You're a helpful personal assistant. Based on the user's message, "
...
def store_message_to_memory(message):
    global messages_memory
...
def sync_chat_completion(par_messages):
    response = client.chat.completions.create(
        # model='deepseek/deepseek-r1-0528:free',
        model='tngtech/deepseek-r1t-chimera:free',
        messages=par_messages,
        max_tokens=1024
    )
...
def query_handling(query):
    ...
    store_message_to_memory({
        'role': 'user',
        'content': query
        })
    
    # Call LLM with the query
    llm_response = sync_chat_completion(get_msg_memory())
    ...
    # This part of the code executes only if LLM returned JSON.
    ...
    tool_response = tool_switch_call(response_tool, response_query)
